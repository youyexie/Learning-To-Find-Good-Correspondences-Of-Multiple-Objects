function [tform_1_2, indices, rmsErr] = ...
    fitRTRansac( ...
    K, ...          % intrinsic matrix
    keypts1_3d,...  % image 1 keypoints 3d, size 3xN
...
N = size(keypts1_3d,2);    % Number of corresponding point pairs
if N<6
    fprintf('Can''t fit a homography using less than 4 point pairs\n');
    return;
end

pts1 = keypts1_3d;  % Get x,y coordinates from image 1, size is (2,N)
...
% Calculate the number of ways to pick a set of 6 points, out of N points.
% This is "N-choose-6"
m = nInlier*(nInlier-1)*(nInlier-2)*(nInlier-3)*(nInlier-4)*(nInlier-5)/factorial(6); % #ways to choose 6 inliers
if m<1  m=1;    end
n = N*(N-1)*(N-2)*(N-3)*(N-4)*(N-5)/factorial(6);  % #ways to choose 6 points

p = m/n;        % probability that any sample of 6 points is all inliers
...
while sample_count < nIterations
    ...
    % Grab 6 matching points at random
    v = randperm(N);
    p1 = pts1(:,v(1:6));
    p2 = pts2(:,v(1:6));
