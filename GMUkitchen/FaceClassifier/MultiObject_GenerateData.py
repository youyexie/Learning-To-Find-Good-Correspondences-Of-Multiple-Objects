...
class MultiObjectDataGenerator(object):
    ...
    Output:
        ...
        Ground truth vec(A)
    """
    def __init__(self,inlier_face = 0,inlier_angle=[0,90]):
        self.k = 0
    ...
    def __getData(self,K,inlierPortion,numData=1000, pixel_std = 5,num_obj=1):        
        ...
        # image point
        P2_img = np.ones([3,numData])
        P2_img[0:1,:] = np.random.randint(10,1920-9,size=[1,numData]) # x 
        P2_img[1:2,:] = np.random.randint(10,1080-9,size=[1,numData]) # y
        ...
        # 2D points in camera coordinate system
        P2 =  np.matmul(np.linalg.inv(K),P2_img)
        
        for i in range(numData):
            temp = np.copy(P2[:,i])
            z = np.random.uniform(3,6)
            
            # make sure X Y Z are in the range [-4,4], [-3,3] and [3,6] 
            while np.abs(temp[0]*z)>4 or np.abs(temp[1]*z)>3:
                z = np.random.uniform(3,6)
        ...
        if num_obj ==0:
            ...
            num_inlier = np.random.randint(0,num_obj)
        ...
        for obj_idx in range(num_obj): 
            ...
            T_now = np.mean( np.matmul(R_inlier,P2[:,curr_idx:curr_idx+obj_num]),axis=1,keepdims=True ) 
            
            P1[:,curr_idx:curr_idx+obj_num] = np.matmul(R_inlier,P2[:,curr_idx:curr_idx+obj_num]) - T_now + T_inlier
...
#            RandomT[2:3,:] = np.random.uniform(4,8,size=[1,1])
            ...
            temp = np.zeros([3,1]);
            temp[0,:] = np.random.uniform(-4,4)
            temp[1,:] = np.random.uniform(-3,3)
            temp[2,:] = np.random.uniform(3,6)
           
            P1[:,curr_idx+i:curr_idx+i+1] = temp#np.zeros([3,1])#np.matmul(RandomR.transpose(),(P2[:,curr_idx+i:curr_idx+i+1]-RandomT))   
...
#        
    ...
    def BatchGetData(self, batch_size=10, numData=1000 ,  pixel_std=5 , max_inlierPortion = 0.3, seed=None , mode='train', test_inlierPortion=0.3,test_num_obj=2  ):
        ...
        #batch_e = np.zeros([batch_size,12])
        
        batch_R = np.zeros([batch_size,3,3,3])
        batch_T = np.zeros([batch_size,3,3,1])
        #Outlier_Portion = np.zeros([batch_size,1])
        
        # define the camera intrinsic matrix
        u0 = 9.5926120509632392e+02
        v0 = 5.2911546499433564e+02
        fx = 1.0477637710998533e+03
